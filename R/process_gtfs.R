...
process_gtfs <- function(gtfs_url, city, date) {
  print(sprintf("\nAnalysing GTFS for %s...", city))
  ...
  destfile <- sprintf("database/transit/%s_gtfs.zip", city)
  ...
  print(sprintf("> Identified %d service patterns matching date: %s", length(pattern_gtfs$.$servicepatterns$servicepattern_id), pattern_gtfs$.$servicepatterns$service_id))
