...
rect.hclust(modelw, 5, border = "orange")
...
member_single <- cutree(models, 5)
member_com <- cutree(modelc, 5)
member_av <- cutree(modela, 5)
member_ward <- cutree(modelw, 5)
member_cen <- cutree(modelcen, 5)
...
km_clust <- kmeans(df_scaled_mean, 5)
